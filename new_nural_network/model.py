...
from new_nural_network.agents import TDAgent, RandomAgent, evaluate_agents
...
class BaseModel(nn.Module):
    ...
    def train_agent(self, env, n_episodes, save_path=None, eligibility=False, save_step=0, name_experiment='', epsilon=None):
        ...
        agents = {WHITE: TDAgent(WHITE, net=network, game_type=env.game_type), BLACK: TDAgent(BLACK, net=network, game_type=env.game_type)}
        ...
        for episode in range(start_episode, n_episodes):
            ...
            if save_path and save_step > 0 and episode > 0 and (episode + 1) % save_step == 0:
                ...
                agents_to_evaluate = {WHITE: TDAgent(WHITE, net=network, game_type=env.game_type), BLACK: RandomAgent(BLACK, game_type=env.game_type)}
...
class TDGammon(BaseModel):
    def __init__(self, hidden_units, game_type, lr, lamda, init_weights, seed=123, input_units=196, output_units=1):
        if game_type == 'short':
            input_units = 198
